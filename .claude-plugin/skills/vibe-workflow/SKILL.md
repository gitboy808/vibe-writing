---
name: vibe-workflow
description: Vibe Writing 统筹入口。当用户用自然语言表达写作意图时触发（"我想写关于XX"、"探索XX"等）。自动识别意图并路由到对应 Agent（学习/结构/写作/成稿）。
version: 0.7.0
---
...
**步骤2-5**: Read 源文档 → 匹配插入位置 → 插入双链 → Edit 保存

**步骤6：处理多张卡片**
逐张处理：生成卡片 → 读取最新源文档 → 匹配 → 插入 → 保存
