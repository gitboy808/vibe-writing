...
**来源链接标注（强制）**：
- WebSearch 获取信息后，必须标注来源链接
- 格式：`[来源标题](URL)`，放在相关内容后的方括号中
- 知识卡片/对话中的引用也需标注来源
...
2. **事实核查**：**使用 Read 工具读取内容** → 识别关键事实点 → 使用 WebSearch 验证 → 输出报告 → 用户确认（如需）
   - **来源链接标注**：WebSearch 验证后，必须在相应内容后标注 `[来源标题](URL)`
...
| 时机 | 文件 | 目的 |
|------|------|------|
| 迭代/润色前 | `项目信息.md` | 获取当前正在优化、输出卡片列表 |
| 迭代整理前 | 当前输出卡片 | 理解现有结构 |
| 润色前 | 用户修改后的输出卡片 | 保留用户修改 |
| 轻量推荐时 | `项目信息.md` | 获取待转化卡片、已输出卡片 |
| 轻量推荐时 | 仅看卡片标题 | 不深度读取内容 |
| 白板组织时 | `内容白板.canvas` | 分析现有连接 |
| 断点续传时 | `项目信息.md` | 获取当前正在优化 |

### 必须更新的文件

| 时机 | 文件 | 更新内容 |
|------|------|----------|
| 开始迭代时 | `项目信息.md` | 设置"当前正在优化" |
| 迭代完成时 | `项目信息.md` | 清空"当前正在优化" |
| 润色完成时 | `项目信息.md` | 迭代次数+1 |
| 迭代完成后 | `内容白板.canvas` | 添加节点 |
| 白板组织后 | `项目信息.md` | 更新白板组织记录 |
| 成稿后 | `项目信息.md` | 可选更新当前阶段 |

### 项目信息.md 字段责任

| 字段 | 责任方 | 更新时机 |
|------|--------|----------|
| 当前正在优化 | writing-agent | 开始迭代时设置，完成后清空 |
| 输出卡片迭代次数 | writing-agent | 每次润色后+1 |
| 白板组织记录 | writing-agent | 白板组织后 |
| 输出卡片列表 | structure-agent（添加）、writing-agent（可选更新） | 生成/优化后 |
